...
function Section({
  ...
  return (
    <Tag
      id={id}
      className={`${className} ${backgroundVariants[background]} ${getSpacingClasses(
        spacing
      )} ${layeredBackground ? "p-2.5" : ""}`}
      style={style}
    >
      {layeredBackground ? (
        <div
          className={`${layeredClassName} ${layeredBackgroundVariants[layeredBackground]} rounded`}
        >
          {children}
        </div>
      ) : (
